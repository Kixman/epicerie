...
if( 'full' !== $layout_structure ){
    $classes[] =  'half_' . ( $full_width === 'true' ? 'fluid' : 'boxed' );
}
...
$global_header_style = $mk_options['theme_header_style'];
$local_override = get_post_meta( $post->ID, '_enable_local_backgrounds', true );
$local_header_style = get_post_meta( $post->ID, 'theme_header_style', true );
$local_header_trans = get_post_meta( $post->ID, '_transparent_header', true );
$local_layout = get_post_meta( $post->ID, '_layout', true );
...
if ( 'full-width' !== $local_layout ) {
	$wrapper_attributes = 'data-mk-full-width="true" data-mk-full-width-init="true" data-mk-stretch-content="' . $stretch_content . '"';
}
?>
